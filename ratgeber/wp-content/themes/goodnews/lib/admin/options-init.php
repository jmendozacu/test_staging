...
if (!class_exists('XT_Redux_Framework_config')) {
    ...
    class XT_Redux_Framework_config {
        ...
        public function initSettings() {
            ...
            add_action( 'redux-page-before-form-'.$this->args['opt_name'], array( $this, 'before_form' ) );
            add_action( 'redux-page-after-form-'.$this->args['opt_name'], array( $this, 'after_form' ) );
        ...
        public function before_form() {
	        ...
	        echo '<div class="xt-redux-form" dir="ltr">';
        }
			...
			if( $wp_filesystem ) {
				...
				if (function_exists('is_plugin_active')) {
					...
					if (is_plugin_active('ubermenu/ubermenu.php')) {
						...
						$content .= "$"."ubermenu: true;\r\n";
					}
					
				}
				
				if(is_rtl()) {
					$content .= "$"."enable_rtl: true;\r\n";
				}
...
*/
        ...
        public function setSections() {
			...
			$xt_sidebars = xt_get_sidebars();
			$widget_zone_layouts = xt_get_widget_layouts();
			
			require_once( XT_WIDGETS_DIR.'/XT_Widget_Advanced_News.php' );
			$news_widget_layouts = XT_Widget_Advanced_News::getLayouts();
						
			$ubermenu_mainmenu_active = xt_ubermenu_location_enabled('main-menu');
			$ubermenu_mobile_active = xt_ubermenu_location_enabled('main-mobile-menu');
			$ubermenu_pushmenu_active = xt_ubermenu_location_enabled('left-side-push-menu') || xt_ubermenu_location_enabled('right-side-push-menu');
			...
			if($currently_in_options && file_exists(XT_ADMIN_DIR . '/redux-pages/import.php')) {
				...
				$currently_in_options = ob_get_contents();
				ob_end_clean();
			}
			...
			$this->sections[] = array(
				...
				'fields' => array(
					...
					array(
						...
						'type' => 'switch',
						'title' => __('Enable Sticky Sidebars', XT_TEXT_DOMAIN),
						'subtitle' => '<p class="description">' . __('This will make page sidebars sticky. A sticky option will also be added to the visual composer column settings which will let you create sticky columns', XT_TEXT_DOMAIN) . '</p>',
						'default' => true
					),
					array(
						'id' => 'hide_admin_bar',
			...
			$this->sections[] = array(
				...
				'fields' => array(
					...
					array(
						...
						'type'  => 'section',
						'title' => __('Top Bar Search', XT_TEXT_DOMAIN),
					),
					...
					array(
						...
						'options' => array(
							...
							'left' => $rtl_mode ? __( 'Right', XT_TEXT_DOMAIN ) : __( 'Left', XT_TEXT_DOMAIN ),
							'right'  => $rtl_mode ? __( 'Left', XT_TEXT_DOMAIN ) : __( 'Right', XT_TEXT_DOMAIN )
						),
						'default' => 'left',
						'compiler' => array(true)
					),
					...
					// Push Menu - General
					array(
						'id'    => 'push_menu_general_settings',
						'type'  => 'section',
						'title' => __('Push Menu - General', XT_TEXT_DOMAIN)
					),
					...
					array(
						...
						'options' => array(
							'disabled'   => __( 'Disabled' , XT_TEXT_DOMAIN ),
							'left' => $rtl_mode ? __( 'Right Menu', XT_TEXT_DOMAIN ) : __( 'Left Menu', XT_TEXT_DOMAIN ),
							'right'  => $rtl_mode ? __( 'Left Menu', XT_TEXT_DOMAIN ) : __( 'Right Menu', XT_TEXT_DOMAIN )
						),
						'default' => 'left',
						'compiler' => array(true)
					),
					...
					array(
						...
						'required' => array( 'push-menu-position', '!=', 'disabled' ),
						'type' => 'text',
						'title' => __('Push Menu Width', XT_TEXT_DOMAIN),
						'default' => '215px',
						'compiler' => array(true)
					),
					...
					array(
						...
						'type'  => 'section',
						'title' => __('Push Menu - Nav Items', XT_TEXT_DOMAIN),
						'subtitle' => $ubermenu_pushmenu_active ? '<p>' . __('This section is disabled / UberMenu is used instead', XT_TEXT_DOMAIN) . '</p>' : '',
						'class' => $ubermenu_pushmenu_active ? 'disabled' : ''
					),
					...
					array(
						...
						'subtitle' => $ubermenu_mainmenu_active ? '<p>' . __('This section is disabled / UberMenu is used instead', XT_TEXT_DOMAIN) . '</p>' : '',
						'class' => $ubermenu_mainmenu_active ? 'disabled' : ''
					),
					...
					array(
						'id'    => 'search_bar_look_and_feel',
						'type'  => 'section',
						'title' => __('Main Menu Search', XT_TEXT_DOMAIN),
						'subtitle' => $ubermenu_mainmenu_active ? '<p>' . __('This section is disabled / UberMenu is used instead', XT_TEXT_DOMAIN) . '</p>' : '',
						'class' => $ubermenu_mainmenu_active ? 'disabled' : ''
					),
					...
					array(
						...
						'type'  => 'section',
						'title' => __('Mobile Menu - General', XT_TEXT_DOMAIN)
					),
					...
					array(
						'id'    => 'mobile_menu_look_and_feel',
						'type'  => 'section',
						'title' => __('Mobile Menu - Nav Items', XT_TEXT_DOMAIN),
						'subtitle' => $ubermenu_mobile_active ? '<p>' . __('This section is disabled / UberMenu is used instead', XT_TEXT_DOMAIN) . '</p>' : '',
						'class' => $ubermenu_mobile_active ? 'disabled' : ''
					),
			...
			$this->sections[] = array(
				...
				'fields' => array(
					...
					array(
						...
						'type' => 'color_rgba',
						'title' => __('Sub Footer Text Color', XT_TEXT_DOMAIN),
						'default' => array(
							'color' => '#606060'
						),
						'compiler' => array(true)
			...
			$comment_systems = array(
				...
				'facebook' => __('Facebook Comments (<a target="_blank" href="https://developers.facebook.com/docs/plugins/comments">More Info</a>) <br><small style="padding-left:24px">NB: Recent Comments widget will not work with this option</small><br><small style="padding-left:24px">NB: You also need to set your Facebook APP ID within the <b>Registrations / Facebook Connect</b> section</small>', XT_TEXT_DOMAIN),
			...
			$this->sections[] = array(
				...
				'fields'     => array(
					...
					array(
						'id' => 'comments_thankyou_pending_msg',
						'required' => array(
							array( 'comments_enabled', '=', '1' ),
							array( 'comments_type', '=', 'wordpress' )
						),
						'type' => 'text',
						'title' => __('Comments Pending Approval Message', XT_TEXT_DOMAIN),
						'default' => __( 'Thank you, your comment has been submitted and is pending approval', XT_TEXT_DOMAIN )
					),
			...
			$this->sections[] = array(
				...
				'fields'     => array(
					...
					array(
                        ...
                        'callback'  => 'xt_likes_reset_all_button'
                    ),
             	)
			);       
            ...
            $this->sections[] = array(
				...
				'icon_class' => 'fa-lg',
				'title' => __('Views Settings', XT_TEXT_DOMAIN),
				'desc' => '<p class="description">' . __('Here are some post views settings that you can edit.', XT_TEXT_DOMAIN) . '</p>',
				'fields' => array(

					array(
						'id'    => 'views_meta_key',
						'type' => 'text',
						'title' => __( 'Views count meta key', XT_TEXT_DOMAIN ),
						'subtitle' => __('You can change the meta key to an existing one if you had an established website with a views system before activating this theme.', XT_TEXT_DOMAIN),
						'default' => 'xt_post_views_count'
					),
					
					array(
						'id'    => 'views_reset_all',
                        'type'   => 'callback',
						'title' => __( 'Reset all views count', XT_TEXT_DOMAIN ),
                        'callback'  => 'xt_views_reset_all_button'
                    )
				)	
					
			);		   
               
            $xtss_providers_names = array();
            $xtss_skins = array();
                    	
            if($currently_in_options) {    
            	global $XT_SocialShare;
				$XT_SocialShare = new XT_SocialShare();
				$xtss_providers_names = $XT_SocialShare->getProvidersNames();
				$xtss_skins = $XT_SocialShare->getSkins();
            }   
            ...
            $this->sections[] = array(
                'title'     => __('API Keys', XT_TEXT_DOMAIN),
                'desc'      => __('Here are couple of Web API settings that you can edit', XT_TEXT_DOMAIN),
                'icon'      => 'el-icon-lock',
                'fields'    => array(
					...
					array(
						...
						'type' => 'text',
						'title' => __('Google Fonts API Key', XT_TEXT_DOMAIN),
						'subtitle' =>  __('This will be used to retrieve all google fonts that will be used within visual composer widgets', XT_TEXT_DOMAIN),
						'default' => ''
					),
                ),
            );    
...
/**
  Reset Likes Callback
 */
if (!function_exists('xt_likes_reset_all_button')) {
	
    function xt_likes_reset_all_button($field, $value) {

        $ajax_nonce = wp_create_nonce( "xt-likes-reset-all" );
...
/**
  Reset Views Callback
 */
if (!function_exists('xt_views_reset_all_button')) {
	
    function xt_views_reset_all_button($field, $value) {

        $ajax_nonce = wp_create_nonce( "xt-views-reset-all" );
	...
	<script type="text/javascript">
		jQuery(document).ready(function($){
			$("#<?php echo esc_js($field['id']);?>").click(function(){
				if ( confirm("Are you want to reset all views?") ) {
					var data = {
						action : 'xt_views_reset_all',
						security : '<?php echo esc_js($ajax_nonce); ?>',
					};
