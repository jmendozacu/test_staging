...
class Flagbit_Mip_Model_Log_Configurator  implements LoggerConfigurationAdapter
{
    ...
    public function convert( $config ) {
            ...
            /* @var $xml Flagbit_Mip_Model_Log_Config */
            $xml = simplexml_load_string( $config->getXmlString() );
            ...
            foreach($xml->loggers as $logger) {
                foreach( $logger as $item ) {
                    $this->parseLogger($item);
                }
            }

            // Process <appender> nodes
            foreach($xml->appenders as $appender ) {
                foreach( $appender as $item ) {
                    $this->parseAppender($item);
                }
            }

            // Process <renderer> nodes
            foreach($xml->renderers as $rendererNode) {
                foreach( $rendererNode as $item ) {
                    $this->parseRenderer($item);
                }
            }

            // Process <defaultRenderer> node
            foreach($xml->defaultRenderers as $rendererNode) {
                foreach( $rendererNode as $item ) {
                    $this->parseDefaultRenderer($item);
                }
            }
    ...
    private function parseParameters($paramsNode) {
        ...
        foreach($paramsNode->params as $paramNodes) {
    ...
    private function parseAppenderReferences(SimpleXMLElement $node) {
        ...
        foreach($node->appenders_ref as $appender_ref) {
            foreach( $appender_ref as $ref ) {
