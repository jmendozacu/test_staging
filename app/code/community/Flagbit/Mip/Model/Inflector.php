 ...
 */
class Flagbit_Mip_Model_Inflector {
 ...
 */
    public function getInstance() {
        ...
        return Mage::getSingleton('mip/inflector');
    }
 ...
 * ));
 * self::rules('transliteration', array('/å/' => 'aa'));
 * }}}
 *
            ...
            default:
                foreach ($rules as $rule => $pattern) {
                    if (is_array($pattern)) {
                        ...
                        if ($type === 'plural') {
                            $this->_pluralized = $this->_tableize = array();
                        } elseif ($type === 'singular') {
                            $this->_singularized = array();
                        }
                    }
                }
 ...
 */
    public function pluralize($word) {
        ...
        if (isset($this->_pluralized[$word])) {
        ...
        if (preg_match('/(.*)\\b(' . $this->_plural['cacheIrregular'] . ')$/i', $word, $regs)) {
            $this->_pluralized[$word] = $regs[1] . substr($word, 0, 1) . substr($this->_plural['merged']['irregular'][strtolower($regs[2])], 1);
            return $this->_pluralized[$word];
        }
        ...
        foreach ($this->_plural['rules'] as $rule => $replacement) {
            if (preg_match($rule, $word)) {
                $this->_pluralized[$word] = preg_replace($rule, $replacement, $word);
                return $this->_pluralized[$word];
            }
        }
    }
 ...
 */
    public function singularize($word) {
        ...
        if (isset($this->_singularized[$word])) {
            return $this->_singularized[$word];
        }

        if (!isset($this->_singular['merged']['uninflected'])) {
            $this->_singular['merged']['uninflected'] = array_merge($this->_singular['uninflected'], $this->_uninflected);
        }

        if (!isset($this->_singular['merged']['irregular'])) {
            $this->_singular['merged']['irregular'] = array_merge($this->_singular['irregular'], array_flip($this->_plural['irregular']));
        }

        if (!isset($this->_singular['cacheUninflected']) || !isset($this->_singular['cacheIrregular'])) {
            $this->_singular['cacheUninflected'] = '(?:' . join( '|', $this->_singular['merged']['uninflected']) . ')';
            $this->_singular['cacheIrregular'] = '(?:' . join( '|', array_keys($this->_singular['merged']['irregular'])) . ')';
        }

        if (preg_match('/(.*)\\b(' . $this->_singular['cacheIrregular'] . ')$/i', $word, $regs)) {
            $this->_singularized[$word] = $regs[1] . substr($word, 0, 1) . substr($this->_singular['merged']['irregular'][strtolower($regs[2])], 1);
            return $this->_singularized[$word];
        }
        ...
        foreach ($this->_singular['rules'] as $rule => $replacement) {
            if (preg_match($rule, $word)) {
                $this->_singularized[$word] = preg_replace($rule, $replacement, $word);
                return $this->_singularized[$word];
            }
        }
        $this->_singularized[$word] = $word;
 ...
 * @deprecated $map param will be removed in future versions. Use self::rules() instead
 * @return string
 * @access public
 * @static
 * @link http://book.cakephp.org/view/1479/Class-methods
 */
    public function slug($string, $replacement = '_', $map = array()) {
        ...
        $map = $map + $this->_transliteration + $merge;
