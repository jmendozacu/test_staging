 ...
 */
class Flagbit_Mip_Model_Errbit_Notice {
    private $_exception;
    private $_options;
    ...
    public function __construct($exception, $options = array()) {
        ...
        $this->_options   = array_merge(
            array(
                ...
                'parameters'   => $request->getParams() !== null ? $request->getParams() : array(),
                'session_data' => array(),
                'cgi_data'     => $this->_getCgiData()
            ),
            $options
        );

        //$this->_options['session_data'] = $this->readLastLinesOfFile(Mage::getBaseDir('var').'/log/mip.log');

        $this->_filterData();
    }

    protected function _getCgiData(){

        return array_merge(
            // Mage::app()->getRequest()->getServer(null, array()),
            array(
                '-MIP_VERSION'       => (string) Mage::getConfig()->getModuleConfig('Flagbit_Mip')->version,
                '-MAGENTO_VERSION'   => Mage::getVersion(),
                '-PHP_VERSION'       => phpversion(),
            ),
            $this->_phpinfoAsArray()
        );
    }

    protected function _phpinfoAsArray()
    {
        ob_start();
        phpinfo();
        $info_arr = array();
    ...
    protected function readLastLinesOfFile($filePath, $lines = 10) {
        $result = null;
    ...
    public static function formatMethod($frame) {
        ...
        if (!empty($frame['class']) && !empty($frame['type']) && !empty($frame['function'])) {
            $method = sprintf('%s%s%s(%s)', $frame['class'], $frame['type'], $frame['function'], $arguments);
        } else {
            $method = sprintf('%s(%s)', !empty($frame['function']) ? $frame['function'] : '<unknown>', $arguments);
        }
        return $method;
    }
    ...
    public static function className($exception) {
        ...
        switch ($name) {
            case 'Flagbit_Mip_Model_Errbit_Errors_Notice':
                return 'Notice';
            case 'Flagbit_Mip_Model_Errbit_Errors_Warning':
                return 'Warning';
            case 'Flagbit_Mip_Model_Errbit_Errors_Error':
                return 'Error';
            case 'Flagbit_Mip_Model_Errbit_Errors_Fatal':
                return 'Fatal Error';
            default:
                return $name;
        }
        ...
        foreach ($array as $key => $value) {
            ...
            if (is_array($value)) {
                $builder->tag('var', array('key' => $key), function($var) use ($value) {
                    Flagbit_Mip_Model_Errbit_Notice::xmlVarsFor($var, $value);
                });
            } else {
    ...
    public function asXml() {
        ...
        $builder   = new Flagbit_Mip_Model_Errbit_XmlBuilder();
        $self      = $this;

        return $builder->tag(
            'notice',
            array('version' => Flagbit_Mip_Model_Errbit::API_VERSION),
            function($notice) use ($exception, $options, $self) {
                $notice->tag('api-key',  $options['api_key']);
                $notice->tag('notifier', function($notifier) {
                    $notifier->tag('name',    Flagbit_Mip_Model_Errbit::PROJECT_NAME);
                    $notifier->tag('version', Flagbit_Mip_Model_Errbit::VERSION);
                    $notifier->tag('url',     Flagbit_Mip_Model_Errbit::PROJECT_URL);
                });

                $notice->tag('error', function($error) use ($exception, $self) {
                    $klass = Flagbit_Mip_Model_Errbit_Notice::className($exception);
                            ...
                            foreach ($trace as $frame) {
                                $backtrace->tag(
                                    ...
                                    array(
                                        ...
                                        'method' => $self->filterTrace(Flagbit_Mip_Model_Errbit_Notice::formatMethod($frame))  ,
                    ...
                    $notice->tag('request', function($request) use ($options) {
                        ...
                        if (!empty($options['parameters'])) {
                            $request->tag('params', function($params) use ($options) {
                                Flagbit_Mip_Model_Errbit_Notice::xmlVarsFor($params, $options['parameters']);
                            });
                        }

                        if (!empty($options['session_data'])) {
                            $request->tag('session', function($session) use ($options) {
                                Flagbit_Mip_Model_Errbit_Notice::xmlVarsFor($session, $options['session_data']);
                            });
                        }

                        if (!empty($options['cgi_data'])) {
                            $request->tag('cgi-data', function($cgiData) use ($options) {
                                Flagbit_Mip_Model_Errbit_Notice::xmlVarsFor($cgiData, $options['cgi_data']);
                            });
                        }
                    });
                }
