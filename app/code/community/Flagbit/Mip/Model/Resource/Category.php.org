...
class Flagbit_Mip_Model_Resource_Category extends Flagbit_Mip_Model_Resource_Abstract {
    ...
    public function create($parentId, $categoryData, $store = null)
    {
        ...
        // handle images
        $this->_handleImages($categoryData, 'image');
        $this->_handleImages($categoryData, 'thumbnail');
     ...
     * usable for type image and thumbnail
     *
     * @param $categoryData
     * @param string $type
     */
    protected function _handleImages(&$categoryData, $type = 'image')
    {
        $path = Mage::getBaseDir('media') . DS . 'catalog' . DS . 'category' . DS;
        if(isset($categoryData[$type])){
            if(is_readable(Mage::getBaseDir().DS.$categoryData[$type]) && is_file(Mage::getBaseDir().DS.$categoryData[$type])){
                $name = substr(strrchr($categoryData[$type], "/"), 1);
                copy(Mage::getBaseDir().DS.$categoryData[$type], $path.$name);
